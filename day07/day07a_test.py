from day07a import get_winnings,get_hand_value,card_counter,card_valuer
...
def test_card_valuer():
    ...
    assert card_valuer('J') == '11', "Test J failed"
...
def test_card_counter():
    assert card_counter('AAAAA') == '7', "Test 5ofk failed"
    assert card_counter('AAAAK') == '6', "Test 40fk failed"
    assert card_counter('AAAKK') == '5', "Test fullhouse failed"
    assert card_counter('AAAKQ') == '4', "Test 3ofk failed"
    assert card_counter('AAKKQ') == '3', "Test 2p failed"
    assert card_counter('AAKQJ') == '2', "Test 1p failed"
    assert card_counter('AKQJT') == '1', "Test 1 failed"
...
def test_hand_value():
    ...
    assert get_hand_value('AAKQJ') == 21414131211, "Test 1p failed"
    assert get_hand_value('AKQJT') == 11413121110, "Test 1 failed"
